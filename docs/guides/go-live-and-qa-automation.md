...
| CD fails: `An asset already exists with this version and published lifecycle state` | Exchange versions are immutable and something republished an existing one | Publish on **tags only**. Any recurring event — a merge to main, a re-dispatch — will eventually run against an unchanged pom version and fail here. Deployment does not need it: the deploy jobs consume the run artifact via `-Dmule.artifact` and never resolve from Exchange. |
